...
const categories = ["Pop","Rock","Blues","Country","Easy Listening","Electronic","Folk","Hip Hop","Jazz","Latin","R&B","Soul"];
window.App = {
  start: function() {
    var self = this;
    var reader;
    ...
    $("#bidding").submit(function(event) {
      ...
      EcommerceStore.deployed().then(function(i) {
        $("#msg").html("Your bid has been submitted. Please wait for few seconds for the confirmation").show();
    ...
    $(".category-link").click(function() {
      renderProducts("product-list", {category: $(this).text()});
    });
...
function buildProduct(product) {
  ...
  node.append("<div>" + displayPrice(product.price) + "</div>");
  node.append("<div class='time'>" + displayEndHours(product.auctionEndTime) + "</div>");
  node.append("<a href='product.html?id=" + product.blockchainId + "' target='_blank' class='details'>Details</a>");
  return node;
}
...
window.web3 = new Web3(new Web3.providers.HttpProvider("https://ropsten.infura.io/YRKuey8Jwty5fs08OILF"));
 } else {
